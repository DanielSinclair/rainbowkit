...
开发者仍然可以完全控制自定义钱包列表，以强调终端用户首选的钱包。 It is encouraged that you continue to include the `injectedWallet` and `walletConnectWallet` in your list to support all platforms.
...
RainbowKit已采用`wagmi`中的`WalletConnectLegacyConnector`连接器，以便继续支持WalletConnect v1。 参考[更新的文档](https://www.rainbowkit.com/docs/custom-wallets)和[示例连接器](packages/rainbowkit/src/wallets/walletConnectors/rainbowWallet/rainbowWallet.ts)以升级您的 dApp 中的任何自定义钱包连接器。
