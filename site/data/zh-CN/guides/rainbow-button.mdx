...
将 `RainbowConnector` 的一个实例传递给您的 Wagmi 连接器列表，并将您的应用包装在 `RainbowButtonProvider` 中。 然后将 `RainbowButton` 组件插入到您的钱包列表中。 然后加入`RainbowButton`组件。 然后加入`RainbowButton`组件。 然后加入`RainbowButton`组件。 Then drop-in the `RainbowButton` component.
