...
> 注意：现在依赖 WalletConnect 的每一个 dApp 都需要从 [WalletConnect Cloud](https://cloud.walletconnect.com/) 获得一个 `projectId`。 这完全免费，只需要几分钟。 这完全免费，只需要几分钟。 这完全免费，只需要几分钟。 这完全免费，只需要几分钟。 这完全免费，只需要几分钟。 这完全免费，只需要几分钟。 这完全免费，只需要几分钟。 这完全免费，只需要几分钟。 这完全免费，只需要几分钟。
...
使用 [Remix](https://remix.run) 时，您必须填充 `buffer`、`events` 和 `http` 模块。 参考下面的 Remix 配置，或者我们的样本 [Remix 项目](https://github.com/rainbow-me/rainbowkit/blob/main/examples/with-remix)。 使用 [Remix](https://remix.run) 时，您必须填充 `buffer`、`events` 和 `http` 模块。 参考下面的 Remix 配置，或者我们的样本 [Remix 项目](https://github.com/rainbow-me/rainbowkit/blob/main/examples/with-remix)。 参考下面的 Remix 配置，或者我们的样本 [Remix 项目](https://github.com/rainbow-me/rainbowkit/blob/main/examples/with-remix)。
