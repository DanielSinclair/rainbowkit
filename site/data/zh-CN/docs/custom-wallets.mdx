...
> 注意：此API不稳定，很可能在不久的将来发生变化。 我们建议现在避免更改钱包列表。 我们将随着时间推移增加更多内置钱包。 注意：此API不稳定，很可能在不久的将来发生变化。 我们建议现在避免更改钱包列表。 我们将随着时间推移增加更多内置钱包。  如果您对任何特定的钱包有兴趣，[请告诉我们](https://github.com/rainbow-me/rainbowkit/issues)。

`钱包`函数类型提供帮助你定义自己的自定义钱包。 `Wallet`函数类型提供帮助你定义自己的自定义钱包。 您的`Wallet`函数返回值上可以配置以下属性： `Wallet`函数类型提供帮助你定义自己的自定义钱包。您的`Wallet`函数返回值上可以配置以下属性： `Wallet`函数类型提供帮助你定义自己的自定义钱包。您的`Wallet`函数返回值上可以配置以下属性： `Wallet`函数类型提供帮助你定义自己的自定义钱包。您的`Wallet`函数返回值上可以配置以下属性：
