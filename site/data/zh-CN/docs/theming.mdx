...
主题函数返回一个主题对象。 主题函数返回一个主题对象。 可以将该对象传递给 `RainbowKitProvider` 的 `theme` 属性。
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      typeSimple: 'enum',
      description:
        "UI中使用的字体堆栈。请注意，`rounded` 尝试使用 SF Pro Rounded，当它不可用时，回退到系统字体。",
    },
    {
      name: 'overlayBlur',
      required: false,
...
/> 注意，`rounded` 尝试使用 SF Pro Rounded 字体，当它不可用时会退回到系统字体。
    },
    {
      name: 'overlayBlur',
      required: false,
