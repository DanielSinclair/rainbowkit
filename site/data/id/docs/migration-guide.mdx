...
Dalam versi sebelumnya dari wagmi yang mengandalkan [ethers](https://docs.ethers.org/v5/), modul `fs`, `net`, dan `tls` yang dibutuhkan oleh WalletConnect secara otomatis dipenuhi polyfill. This is no longer the case with RainbowKit v1 + wagmi v1, which are built on [viem](https://viem.sh/).
