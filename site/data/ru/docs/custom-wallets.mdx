...
<PropsTable
  data={[
    ...
    },
    {
      ...
      description:
        'Функция для расчета, нужно ли скрыть этот кошелек из списка.',
    } 
  ]}
/> Это полезно для определения общих резервных кошельков, например, встроенная функция резервного копирования "Injected Wallet" использует эту возможность',
    },
    {
