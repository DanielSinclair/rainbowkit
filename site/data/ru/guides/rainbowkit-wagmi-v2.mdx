...
Разработчики по-прежнему имеют полный контроль над Списком пользовательских кошельков, чтобы акцентировать внимание на предпочтительных кошельках для конечных пользователей. It is encouraged that you continue to include the `injectedWallet` and `walletConnectWallet` in your list to support all platforms.
