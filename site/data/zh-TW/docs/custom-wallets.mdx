...
> 注意：此API不穩定，近期可能會變更。 我們將隨著時間添加更多內建錢包。 [讓我們知道](https://github.com/rainbow-me/rainbowkit/issues) 您感興趣的特定錢包。

提供 `Wallet` 函數類型，幫助您定義自己的自訂錢包。 您的 `Wallet` 函數的返回值可以配置以下屬性：
...
然後，您可以在 [錢包列表](/docs/custom-wallet-list) 中添加您的自訂錢包。
