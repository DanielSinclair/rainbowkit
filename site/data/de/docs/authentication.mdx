...
Während es möglich ist, mit benutzerdefinierten Back-Ends und Nachrichtenformaten zu [integrieren,](/docs/custom-authentication) bietet RainbowKit erstklassige Unterstützung für [Sign-In with Ethereum](https://login.xyz) und [NextAuth.js.](https://next-auth.js.org)

### Sign-In with Ethereum und NextAuth.js einrichten

Wenn noch nicht geschehen, richten Sie zunächst Ihr [Next.js](https://nextjs.org) Projekt mit dem [offiziellen Sign-In mit Ethereum-Vorlagenprojekt für NextAuth.js](https://docs.login.xyz/integrations/nextauth.js) ein.

#### Installieren

Installieren Sie das `@rainbow-me/rainbowkit-siwe-next-auth` Paket und seine Peer-Abhängigkeit, [ethers](https://docs.ethers.org/v5/).

```bash
npm install @rainbow-me/rainbowkit-siwe-next-auth siwe@^2 ethers@^5
```

> Hinweis: `siwe` erfordert die [ethers](https://docs.ethers.org/v5/) Peer-Abhängigkeit, während [wagmi](https://wagmi.sh/) nun auf die Alternative [viem](https://viem.sh) setzt.
...
Sie können die [SIWE-Nachrichtenoptionen](https://github.com/spruceid/siwe/blob/v1.1.6/packages/siwe/lib/client.ts#L29) anpassen, indem Sie eine Funktion an die `getSiweMessageOptions` Eigenschaft in `RainbowKitSiweNextAuthProvider` übergeben.
