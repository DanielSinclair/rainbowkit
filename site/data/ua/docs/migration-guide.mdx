...
У попередніх версіях wagmi, які використовували [ethers](https://docs.ethers.org/v5/), модулі `fs`, `net` та `tls`, необхідні для WalletConnect, автоматично поліфілувалися. This is no longer the case with RainbowKit v1 + wagmi v1, which are built on [viem](https://viem.sh/).
